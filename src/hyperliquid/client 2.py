...
class HyperliquidClient:
    ...
    def _sign_l1_action(self, action: Dict[str, Any], nonce: int) -> Dict[str, Any]:
        ...
        # This is simplified - actual implementation would need full EIP-712 structure
        message = encode_defunct(text=json.dumps(action))
    ...
    async def get_funding_rate(self, asset: str) -> DerivativesData:
        ...
        # Get open interest
        oi_response = await self._request("POST", "/info/openInterest", data)
        open_interest = float(oi_response.get('openInterest', 0))

        # Get historical funding to determine trend
        funding_history = response.get('history', [])
    ...
    async def get_account_state(self) -> Portfolio:
        ...
        endpoint = "/info/accountState"
        response = await self._request("POST", endpoint, authenticated=True)
    ...
    async def place_order(
        ...
        logger.info(f"Placing {order_type} {side} order: {size} {asset} @ {price}")

        try:
            response = await self._request("POST", endpoint, order_data, authenticated=True)
    ...
    async def get_trade_history(self, asset: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        endpoint = "/info/trades"
        data = {"coin": asset, "limit": limit}
        response = await self._request("POST", endpoint, data)
